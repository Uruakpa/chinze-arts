#!/usr/bin/env python3
"""
Update App.tsx with new gallery items that include dimensions and update lightbox format
"""
...
# Find the section from echoes-solitude-08 to whispers-trees-35 and replace it
old_section_start = '  {\n    id: "echoes-solitude-08",'
old_section_end = '    section: "whispers",\n  },'

# Find the index of these markers
start_idx = app_content.find(old_section_start)
end_idx = app_content.find(old_section_end) + len(old_section_end)

if start_idx == -1 or end_idx == -1:
    print("Could not find the section to replace")
    exit(1)

# New items with dimensions
new_items = '''  {
    id: "echoes-solitude-08",
...
# Replace the section
new_content = app_content[:start_idx] + new_items + app_content[end_idx:]

# Now update the lightbox display format to show: Title / Medium / Dimensions / Year
# Find the lightbox metadata section
lightbox_pattern = r'<div className="bg-white p-6 md:p-8">\s*<h2 className="font-serif text-2xl md:text-3xl font-semibold mb-4">\s*\{activeItem\.year\}.*?\{activeItem\.medium\}\s*</div>'

# Search for the lightbox section
lightbox_start = new_content.find('activeItem && (')
if lightbox_start != -1:
    # Find the metadata section
    metadata_start = new_content.find('<div className="bg-white p-6 md:p-8">', lightbox_start)
    if metadata_start != -1:
        # Find the end of the h2 section
        h2_end = new_content.find('</div>', metadata_start)
        if h2_end != -1:
            # Count to find the right closing div
            h2_end = new_content.find('</div>', metadata_start + 50) + 6
            
            # Create the new metadata section
            new_metadata = '''<div className="bg-white p-6 md:p-8">
          <h2 className="font-serif text-2xl md:text-3xl font-semibold mb-4">
            {activeItem.title} {activeItem.medium ? `/ ${activeItem.medium}` : ''} {activeItem.dimensions ? `/ ${activeItem.dimensions}` : ''} {activeItem.year ? `/ ${activeItem.year}` : ''}
          </h2>'''
            
            # Replace the section
            new_content = new_content[:metadata_start] + new_metadata + new_content[h2_end:]

# Write the updated content
with open('src/App.tsx', 'w') as f:
    f.write(new_content)

print("Updated App.tsx with new gallery items and lightbox format")
